...
def calc_features(coco_train, img_count, img_captions, tagsPerImage):
    ...
    for image_id, words in bar(img_count.items()):
        ...
        if pos % 20000 == 0:
            ...
            np.savez_compressed('features/train_features', img_features=img_features[:TAGS_PER_IMAGE * pos,:], tag_features=tag_features[:TAGS_PER_IMAGE * pos,:])

    print('Training: saving features calculated for all the images')
    np.savez_compressed('features/train_features', img_features=img_features, tag_features=tag_features)
