...
class CaptionDataset(Dataset):
    def __init__(self, data_folder, data_name, split, transform=None):
        ...
        with open(os.path.join(data_folder, self.split + '_ATTRIBUTES_' + data_name + '.json'), 'r') as j:
            self.attributes = json.load(j)    
            
        # Load image url
        with open(os.path.join(data_folder, self.split + '_IMG_URLS_' + data_name + '.json'), 'r') as j:
            self.imgurls = json.load(j)    
    ...
    def __getitem__(self, i):
        ...
        attributes = torch.LongTensor(self.attributes[i // self.cpi])
        
        imgurl = self.imgurls[i // self.cpi]
        
        if self.split is 'TRAIN':
            return imgurl, img, caption, caplen, attributes
        else:
            # For validation of testing, also return all 'captions_per_image' captions to find BLEU-4 score
            all_captions = torch.LongTensor(
                self.captions[((i // self.cpi) * self.cpi):(((i // self.cpi) * self.cpi) + self.cpi)])
            return imgurl, img, caption, caplen, attributes, all_captions
